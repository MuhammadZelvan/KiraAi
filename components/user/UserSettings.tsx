...
import { ChevronDown, Save, Copy, MoreHorizontal, Check, Loader2 } from "lucide-react";
...
export function UserSettings() {
    ...
    const [searchEnabled, setSearchEnabled] = useState(() => loadBool("searchEnabled", true));
    const [voiceProfile, setVoiceProfile] = useState(() => load("voiceProfile", "alloy"));
    const [speechSpeed, setSpeechSpeed] = useState(() => load("speechSpeed", "1.0"));
    const [autoPlayVoice, setAutoPlayVoice] = useState(() => loadBool("autoPlayVoice", false));

    // Saved snapshots used for dirty detection
    const [saved, setSaved] = useState({ fullName, callName, occupation, preferences, notifyCompletion, colorMode, chatFont, searchEnabled, voiceProfile, speechSpeed, autoPlayVoice });
    ...
    }, [fullName, callName, occupation, preferences, notifyCompletion, colorMode, chatFont, searchEnabled, voiceProfile, speechSpeed, autoPlayVoice, saved]);
    ...
    const handleSave = () => {
        ...
        setTimeout(() => {
            ...
            const vals: Record<string, string> = {
                ...
                colorMode, chatFont,
                searchEnabled: String(searchEnabled),
                voiceProfile, speechSpeed,
                autoPlayVoice: String(autoPlayVoice),
            };
            Object.entries(vals).forEach(([k, v]) => localStorage.setItem(`lyra_${k}`, v));

            setSaved({ fullName, callName, occupation, preferences, notifyCompletion, colorMode, chatFont, searchEnabled, voiceProfile, speechSpeed, autoPlayVoice });
            setIsSaving(false);
            setJustSaved(true);
            setIsDirty(false);
    ...
    return (
        <div className="flex justify-center h-full bg-background overflow-y-auto w-full relative">
            <div className="flex flex-col md:flex-row w-full max-w-5xl px-4 md:px-8 py-10 gap-x-12">
                ...
                <div className="flex-1 w-full max-w-3xl pb-24 relative">
                    {activeTab === "general" && (
                        <div className="space-y-10 animate-in fade-in duration-300">
                                ...
                                <div className="flex items-start justify-between">
                                    <div>
                                        ...
                                        <p className="text-[13px] text-muted-foreground mt-1 max-w-[85%]">
                                            Get notified when LyraAI has finished a response. Most useful for long-running tasks like tool calls and Research.
                                        </p>
                                    </div>
                                    <Switch
                                        ...
                                        <div className="flex flex-col items-center gap-3">
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    colorMode === "light" ? "border-primary ring-1 ring-primary" : "border-border/60 hover:border-foreground/30"
                                                )}
                                            >
                                        ...
                                        <div className="flex flex-col items-center gap-3">
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    colorMode === "auto" ? "border-primary ring-1 ring-primary" : "border-border/60 hover:border-foreground/30"
                                                )}
                                            >
                                        ...
                                        <div className="flex flex-col items-center gap-3">
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    colorMode === "dark" ? "border-primary ring-1 ring-primary" : "border-border/60 hover:border-foreground/30"
                                                )}
                                            >
                                        ...
                                        <div key={font} className="flex flex-col items-center gap-3">
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    chatFont === font ? "border-primary ring-1 ring-primary bg-primary/5" : "border-border/60 hover:border-foreground/30"
                                                )}
                                            >
                                                <span className={cn(
                                                    "text-[28px] text-foreground/90",
                                                    font === 'Sans' ? "font-sans" : font === 'System' ? "font-system" : "font-serif"
                                                )}>Aa</span>
                                            </button>
                            ...
                            <div className="space-y-6 pb-4">
                                <div>
                                    <h3 className="text-base font-bold text-foreground tracking-tight">AI Voice Settings</h3>
                                    <p className="text-sm text-muted-foreground mt-1">Configure how LyraAI communicates with you via audio.</p>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                                    {/* Voice Profile Dropdown */}
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium text-foreground">Voice Profile</label>
                                        <div className="relative">
                                            <select
                                                value={voiceProfile}
                                            ...
                                            >
                                                ...
                                                <option value="fable">Fable (Animated, Expressive)</option>
                                                <option value="nova">Nova (Bright, Professional)</option>
                                            </select>
                                            <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground pointer-events-none" />
                                        </div>
                                        <p className="text-xs text-muted-foreground mt-1">Select the AI's speaking voice.</p>
                                    </div>
                                ...
                                <div className="flex items-center justify-between pt-2">
                                    <div>
                                        <h4 className="text-sm font-medium text-foreground">Auto-play audio responses</h4>
                                        <p className="text-[13px] text-muted-foreground mt-1 max-w-[85%]">
                        ...
                        <div className="space-y-8 animate-in fade-in duration-300">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex items-center justify-between py-1">
                                    ...
                                    <Button variant="outline" size="sm" className="rounded-md">Log out</Button>
                                </div>
                                ...
                                <div className="flex items-center justify-between py-1 border-t border-border/40 pt-5">
                                    ...
                                    <div className="flex items-center gap-3 bg-muted/40 px-3 py-1.5 rounded-md border border-border/50">
                                        <span className="text-xs font-mono text-muted-foreground">3335a215-5c87-4ff4-a29b-031c15e15c50</span>
                            ...
                            <div className="mt-14 pt-6 border-t border-border/40">
                                <h3 className="text-[15px] font-bold text-foreground mb-4">Active sessions</h3>
                                <div className="w-full overflow-x-auto">
                                    <table className="w-full text-left text-sm whitespace-nowrap">
                                        <thead>
                                            <tr className="border-b border-border/60 text-muted-foreground">
                                                <th className="pb-3 font-medium min-w-[180px]">Device</th>
                                                <th className="pb-3 font-medium min-w-[200px]">Location</th>
                                                <th className="pb-3 font-medium min-w-[160px]">Created</th>
                                                <th className="pb-3 font-medium min-w-[160px]">Updated</th>
                                                <th className="pb-3 font-medium w-10"></th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-border/30">
                                            <tr className="hover:bg-muted/10 transition-colors">
                                                <td className="py-4 font-medium text-foreground">Chrome Mobile (Android)</td>
                                                <td className="py-4 text-muted-foreground">Purwokerto, Central Java, ID</td>
                                                <td className="py-4 text-muted-foreground">Mar 1, 2026, 10:57 AM</td>
                                                <td className="py-4 text-muted-foreground">Mar 2, 2026, 11:16 AM</td>
                                            ...
                                            <tr className="hover:bg-muted/10 transition-colors">
                                                <td className="py-4 font-medium text-foreground flex items-center gap-2">
                                                    Firefox (Windows)
                                                    <span className="bg-muted text-[10px] px-1.5 py-0.5 rounded text-muted-foreground border border-border/50">Current</span>
                                                </td>
                                                <td className="py-4 text-muted-foreground">Bojonegoro, East Java, ID</td>
                                                <td className="py-4 text-muted-foreground">Jan 15, 2026, 10:23 AM</td>
                                                <td className="py-4 text-muted-foreground">Mar 2, 2026, 8:04 AM</td>
                                                <td className="py-4 text-right">
                                                    <button className="text-muted-foreground hover:text-foreground">
                                                        <MoreHorizontal className="h-4 w-4 inline-block" />
                                                    </button>
                    ...
                    {activeTab === "capabilities" && (
                        <div className="space-y-8 animate-in fade-in duration-300">
                            <h2 className="text-xl font-bold text-foreground mb-8">Capabilities & Features</h2>

                            <div className="space-y-8">
                                <div className="flex items-center justify-between py-2">
                                    <div>
                                        <h3 className="text-sm font-medium text-foreground">Web Search</h3>
                                        <p className="text-xs text-muted-foreground mt-1 max-w-[85%] leading-relaxed">Allow LyraAI to autonomously browse the internet to find up-to-date information when your questions require current events or factual accuracy.</p>
                                    </div>
                                    <Switch
                                ...
                                <div className="flex items-center justify-between py-2 border-t border-border/40 pt-6">
                                    <div>
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1 max-w-[85%] leading-relaxed">Enable the ability for LyraAI to generate images using stable diffusion models when requested in chat.</p>
                                    </div>
                                    <Switch
                                        checked={true}
                                    />
                                ...
                                <div className="flex items-center justify-between py-2 border-t border-border/40 pt-6">
                                    <div>
                                        <h3 className="text-sm font-medium text-foreground">Code Interpreter</h3>
                                        <p className="text-xs text-muted-foreground mt-1 max-w-[85%] leading-relaxed">Allow the AI to write and execute Python code in a sandboxed environment to solve math problems, analyze data, or create files.</p>
                                    </div>
                                    <Switch
                                        checked={false}
                                    />
            ...
            <div className={cn(
                ...
                isDirty || justSaved ? "translate-y-0 opacity-100" : "translate-y-12 opacity-0 pointer-events-none"
            )}>
                <Button
                    onClick={handleSave}
